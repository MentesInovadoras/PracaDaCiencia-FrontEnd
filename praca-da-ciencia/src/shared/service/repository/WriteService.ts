...
export class WriteService<
  ReadType extends BaseEntity,
  WriterType extends BaseEntity = ReadType
> extends BaseService {
  ...
  public constructor(
    entityName: string,
    serializeStrategy: ISeralizerStrategy<
      ReadType,
      WriterType
    > = new ParseMultiEntityStrategy(),
    id?: ID
  ) {
    super(entityName);
   ...
   */
  public setSerializeStrategy(
    strategy: ISeralizerStrategy<ReadType, WriterType>
  ) {
   ...
   */
  public async fetchEntity(id: ID) {
  ...
  setEntity(data: ReadType) {
    const serializedEntity = this.serializer.serialize(data);
    for (const key in this.mustUpdate) {
      this.mustUpdate[key] = true;
    }

    Object.assign(this.entity, serializedEntity);
  }
   ...
   */
  public setEntityKey(key: keyof ReadType, newValue: ReadType[keyof ReadType]) {
  ...
  public entityExists(): boolean {
    return (
      (typeof this.entity.id === "string"
        ? Number.parseInt(this.entity.id)
        : this.entity.id) >= 0
    );
  }
  ...
  private async create(): GenericAxiosResponse {
    ...
    const response = await api.post(
      this.url,
      this.serializer.serialize(this.entity)
    );
  ...
  private async economicUpdate(): GenericAxiosResponse {
    ...
    const serializedData = this.serializer.serialize(this.entity);

    for (const key in this.mustUpdate) {
      if (this.mustUpdate[key]) {
        // @ts-ignore
        serialize[key] = serializedData[key];
      }
    }
    this.cleanUpdate();
